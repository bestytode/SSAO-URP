Shader "Hidden/Universal Render Pipeline/UberPost"
{
    HLSLINCLUDE
        ...
        TEXTURE2D(_SSAO_Texture); // custom ssao texture

        float _CUSTOM_SSAO_ENABLED; // 1.0 for enabled, 0.0 for disabled
        float4 _Lut_Params;
        ...
        half4 FragUberPost(Varyings input) : SV_Target
        {
            ...
            // TODO: not apply occlusion factor after post-process, but after g-buffer with ambient light.
            if(_CUSTOM_SSAO_ENABLED > 0){
            float4 ssao = SAMPLE_TEXTURE2D(_SSAO_Texture, sampler_LinearClamp, input.texcoord);
            color *= ssao;
            }
